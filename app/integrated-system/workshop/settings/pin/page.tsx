          ...
          {row.map((digit, colIdx) => {
            ...
            if (rowIdx === 3 && colIdx === 2) {
              return showSubmit ? (
                ...
                >
                  {disabled ? <Loader2 className="h-5 w-5 animate-spin" /> : <Check className="h-5 w-5" strokeWidth={2.5} />}
                </button>
              ) : (
...
export default function PinSettingsPage() {
  ...
  const autoAdvance = useCallback(async () => {
    ...
    if (step === "enter" && (mode === "set" || mode === "change")) {
  ...
  const renderPinPad = (label: string, showSubmit: boolean) => (
    <div ref={containerRef} tabIndex={0} onKeyDown={handleKeyDown} className="outline-none">
        <p className="mb-6 text-center text-[11px] uppercase tracking-wider text-stone-400">{label}</p>
        <PinDots filled={pin.length} />
        <div className="mt-4 mb-2 text-center text-[12px] text-stone-400">{pin.length}/{PIN_LENGTH} digit</div>
        {errMsg && (
          <div className="mb-6 rounded-lg bg-red-50 px-4 py-2.5 text-center text-[13px] text-red-600 border border-red-100">{errMsg}</div>
        )}
        <Numpad onDigit={handleDigit} onClear={handleClear} onSubmit={autoAdvance} disabled={submitting} showSubmit={showSubmit} />
        <div className="mt-6 text-center">
          <button type="button" onClick={goBack} disabled={submitting} className="text-[13px] text-stone-400 underline underline-offset-2 transition-colors hover:text-stone-600 disabled:opacity-40">
            ← Kembali
          </button>
      </div>
    </div>
    ...
    return (
      <div className="w-full max-w-[380px] text-center">
        ...
        <div className="rounded-2xl border border-red-100 bg-white/90 p-7 shadow-sm">
          ...
          <button onClick={() => router.push("/integrated-system/dashboard/worker")} className="text-[14px] font-medium text-amber-600 hover:text-amber-700">
    ...
    return (
      <div className="w-full max-w-[380px] text-center">
        ...
        <div className="rounded-2xl border border-emerald-100 bg-white/90 p-8 shadow-sm">
          ...
          <p className="text-[13px] text-stone-500 mb-6">{mode === "set" ? "PIN berhasil diatur." : "PIN berhasil diperbarui."}</p>
          <button onClick={() => router.push("/integrated-system/dashboard/worker")} className="rounded-xl bg-stone-800 px-6 py-2.5 text-[14px] font-medium text-white shadow-sm hover:bg-stone-900 active:scale-[0.98] transition-all">
            Kembali ke Dashboard
          </button>
        </div>
      </div>
      ...
      <div className="w-full max-w-[380px]">
        ...
        <div className="rounded-2xl border border-stone-200/80 bg-white/90 backdrop-blur-sm shadow-sm p-6 text-center">
          ...
          <div className="mt-6 space-y-3">
            {user?.pin_hash ? (
              <button onClick={startChange} className="w-full rounded-xl bg-stone-800 py-3 text-[14px] font-medium text-white shadow-sm hover:bg-stone-900 active:scale-[0.98] transition-all">Ubah PIN</button>
            ) : (
              <button onClick={startSet} className="w-full rounded-xl bg-stone-800 py-3 text-[14px] font-medium text-white shadow-sm hover:bg-stone-900 active:scale-[0.98] transition-all">Atur PIN</button>
            )}
            <button onClick={() => router.push("/integrated-system/dashboard/worker")} className="w-full rounded-xl border border-stone-200 bg-white py-3 text-[14px] font-medium text-stone-600 hover:bg-stone-50 active:scale-[0.98] transition-all">
              Kembali ke Dashboard
            </button>
          </div>
        </div>
