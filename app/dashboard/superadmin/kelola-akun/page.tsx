...
export default function KelolaAkunPage() {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      const payload = {
        ...
        branch_id: ["cs"].includes(userFormData.role)
          ? userFormData.branch_id || null
          : null,
  ...
  const handleToggleUserStatus = async (user: User) => {
    ...
    showAlert(
      "success",
      `Akun ${newStatus === "active" ? "diaktifkan" : "dinonaktifkan"}!`,
    );
    await fetchUsers();
  };
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                  {[
                    ...
                    {
                      label: "Super Admin",
                      count: users.filter((u) => u.role === "superadmin")
                        .length,
                      icon: "👑",
                      bg: "bg-purple-100",
                    },
                    {
                      label: "CS",
                      count: users.filter((u) => u.role === "cs").length,
                      icon: "💬",
                      bg: "bg-blue-100",
                    },
                    {
                      label: "Marketing",
                      count: users.filter((u) => u.role === "marketing").length,
                      icon: "📊",
                      bg: "bg-green-100",
                    },
