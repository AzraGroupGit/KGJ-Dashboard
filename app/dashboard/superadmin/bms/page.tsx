...
function DeltaBadge({ delta, unit = "" }: { delta: number; unit?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-mono font-medium px-2 py-0.5 rounded-full ${up ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-500"}`}
    >
      {up ? "▲" : "▼"} {Math.abs(delta)}
      {unit} vs kemarin
    </span>
    ...
    <div className="flex items-end gap-1.5 h-28">
      {trend.map((t, i) => {
        ...
        return (
          <div
            key={t.date}
            className="flex flex-col items-center flex-1 h-full gap-1"
          >
            <span className="text-[9px] font-mono text-slate-400 h-4 flex items-center">
              {fmt(vals[i])}
            </span>
            ...
            <span
              className={`text-[10px] font-mono whitespace-nowrap ${isLast ? "text-indigo-500 font-semibold" : "text-slate-400"}`}
            >
              {t.label}
            </span>
          </div>
...
function KpiCard({
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-hidden before:absolute before:top-0 before:left-0 before:right-0 before:h-[3px] before:rounded-t-2xl ${accent[color] ?? ""}`}
    >
      <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 mb-2">
        {title}
      </p>
      <p className="text-[22px] font-bold text-slate-900 leading-none mb-1.5">
        {value}
      </p>
      {sub && (
        <p className="text-[11px] text-slate-400 font-mono mb-2">{sub}</p>
      )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <KpiCard
                      ...
                      sub={
                        data.totals.closing > 0
                          ? `${fmtRpShort(data.totals.omset / data.totals.closing)} / closing`
                          : "—"
                      }
                      color="amber"
                    ...
                    <KpiCard
                      ...
                      sub={
                        data.totals.omset > 0
                          ? `${fmtPct((data.totals.gross_profit / data.totals.omset) * 100)} margin`
                          : "0% margin"
                      }
                      color="rose"
                    />
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <KpiCard
                      ...
                      value={
                        data.totals.closing > 0
                          ? fmtRpShort(data.totals.omset / data.totals.closing)
                          : "—"
                      }
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {data.staff
                            ...
                            .map((s, i) => {
                              ...
                              const rankColor =
                                rankColors[i] ?? "bg-slate-50 text-slate-400";
                              return (
                                <tr
                                  key={s.id}
                                ...
                                >
                                  <td className="px-5 py-3.5">
                                    <span
                                      className={`w-6 h-6 inline-flex items-center justify-center text-[11px] font-bold rounded-md ${rankColor}`}
                                    >
                                      {i + 1}
                                    </span>
                                  </td>
                                  ...
                                  <td className="px-5 py-3.5">
                                    <div className="flex items-center gap-2">
                                      <span
                                        className={`text-xs font-semibold ${staffCr > 30 ? "text-emerald-600" : staffCr > 15 ? "text-amber-500" : "text-rose-500"}`}
                                      >
                                        {fmtPct(staffCr)}
                                      </span>
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                          {
                            label: "Lead Masuk",
                            field: "lead_masuk" as const,
                            color: "#6366f1",
                            fmt: (v: number) => String(v),
                          },
                          {
                            label: "Closing",
                            field: "closing" as const,
                            color: "#10b981",
                            fmt: (v: number) => String(v),
                          },
                          {
                            label: "Omzet",
                            field: "omset" as const,
                            color: "#f59e0b",
                            fmt: (v: number) =>
                              v >= 1_000_000
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-slate-50">
                              {[...data.trend].reverse().map((t) => {
                                const rowCr =
                                  t.lead_masuk > 0
                                    ? (t.closing / t.lead_masuk) * 100
                                    : 0;
                                const isSelected = t.date === selectedDate;
                                return (
                                  <tr
                                    key={t.date}
                                    className={`transition-colors ${isSelected ? "bg-indigo-50/60" : "hover:bg-slate-50"}`}
                                  >
                                    <td className="px-5 py-3.5">
                                      <div className="flex items-center gap-2">
                                        {isSelected && (
                                          <span className="text-[10px] font-bold bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded">
                                            {t.date === toLocalDate(new Date())
                                              ? "HARI INI"
                                              : "DIPILIH"}
                                          </span>
                                        )}
                                    ...
                                    <td className="px-5 py-3.5">
                                      <span
                                        className={`text-xs font-semibold ${rowCr > 30 ? "text-emerald-600" : rowCr > 15 ? "text-amber-500" : "text-rose-500"}`}
                                      >
                                        {fmtPct(rowCr)}
                                      </span>
                                    </td>
