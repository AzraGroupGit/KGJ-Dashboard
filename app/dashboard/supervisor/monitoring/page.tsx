...
const STAGE_COLORS: Record<string, string> = {
  ...
  other: "bg-white/10 text-cream border-gold/15",
};
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-lg border border-gold/15 bg-cocoa p-3 sm:p-5${className ? ` ${className}` : ""}`}
    >
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 sm:mt-2 text-xl sm:text-3xl font-semibold tabular-nums text-ivory">
            {value}
          </p>
          {note && (
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6">
            ...
            <div className="flex items-center justify-between sm:justify-end gap-2">
              {data?.stats.pendingApprovals ? (
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-md bg-amber-500/10 border border-amber-400/20 px-2.5 sm:px-3 py-1.5 text-[10px] sm:text-xs font-medium text-amber-300 hover:bg-amber-500/100/10 transition-colors whitespace-nowrap"
                >
          ...
          {isLoading ? (
            ...
            <div className="space-y-4 sm:space-y-6">
              {/* Hari Ini */}
              <div>
                <h3 className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/40 mb-2">
                  Hari Ini
                </h3>
              ...
              {filteredOrders.length === 0 ? (
                ...
                <>
                      ...
                      if (isCompleted) {
                        return (
                          <button
                            ...
                            className="w-full text-left rounded-lg border border-gold/15 bg-cocoa p-3 sm:p-4 active:bg-[#26211c] transition-colors hover:border-gold/25"
                          >
                            <div className="flex items-start justify-between gap-2 mb-1.5">
                              <div className="min-w-0">
                                <span className="font-mono text-xs sm:text-sm font-semibold text-cream">
                                  {order.order_number}
                                </span>
                                {order.customer_name && (
                              ...
                              <span className="inline-block shrink-0 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-200 px-2 py-0.5 text-[10px] sm:text-[11px] font-medium">
                                Selesai
                              </span>
                            </div>
                            <div className="flex items-center justify-between text-xs gap-2 flex-wrap mt-1.5">
                              ...
                              <div className="flex items-center gap-2">
                                {processLabel && (
                                  <span className="text-white/40">{processLabel}</span>
                                )}
                                {onTime !== null ? (
                                  onTime
                                    ? <span className="text-emerald-300 font-medium">Tepat Waktu</span>
                                    : <span className="text-rose-300 font-medium flex items-center gap-1"><AlertTriangle className="h-3 w-3" />Terlambat</span>
                                ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between text-xs gap-2 flex-wrap mt-1.5">
                            ...
                            <div className="flex items-center gap-2">
                              {hoursLabel !== "—" && (
                                <span className={`text-[10px] sm:text-[11px] font-medium ${isStuck ? "text-rose-300" : "text-white/40"}`}>
                                  {isStuck && <Clock className="inline h-3 w-3 mr-0.5" />}
                                  {hoursLabel}
                                </span>
                              )}
                              <span className={dl.urgent ? "font-semibold text-rose-300" : "text-white/40"}>
                  ...
                  {/* Desktop table */}
                  <div className="hidden md:block rounded-lg border border-gold/15 bg-cocoa overflow-hidden">
                          ...
                          <tr className="border-b border-gold/10 bg-[#26211c]/70">
                            ...
                            {filter === "completed" ? (
                              ...
                              <>
                                ...
                                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-white/50 hidden lg:table-cell">
                                  Lama di Tahap
                                </th>
                                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-white/50 hidden lg:table-cell">
                                  Deadline
                              ...
                              return (
                                ...
                                >
                                  <td className="px-4 py-3">
                                    <span className="font-mono text-xs font-semibold text-cream">{order.order_number}</span>
                                    {order.customer_name && <p className="text-[11px] text-white/70 mt-0.5 font-medium">{order.customer_name}</p>}
                                  </td>
                                  <td className="px-4 py-3">
                                  ...
                                  <td className="px-4 py-3">
                                    <span className="text-cream text-xs">{processLabel ?? "—"}</span>
                                  </td>
                                  <td className="px-4 py-3">
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  {order.last_worker ? (
                                    <div>
                                      <span className="text-cream">{order.last_worker}</span>
                                      <p className="text-[11px] text-white/40 mt-0.5">{formatRelative(order.last_submission_at)}</p>
                                    </div>
                                  ) : (
                                    <span className="text-white/30 text-xs">Belum ada submission</span>
                                  )}
                                </td>
                                <td className="px-4 py-3 hidden lg:table-cell">
                                  {hoursLabel !== "—" ? (
                                    <div>
                                      <span className={`text-sm font-medium ${isStuck ? "text-rose-300" : "text-white/70"}`}>
                                        {isStuck && <Clock className="inline h-3.5 w-3.5 mr-1 -mt-0.5" />}
                                        {hoursLabel}
                                      </span>
                                      <p className="text-[11px] text-white/40 mt-0.5">{order.stage_label}</p>
                                    </div>
                                  ) : (
                                    <span className="text-xs text-white/30">—</span>
                                  )}
                                ...
                                <td className="px-4 py-3 hidden lg:table-cell">
                                  <span className={`text-sm ${dl.urgent ? "font-semibold text-rose-300" : "text-white/70"}`}>
