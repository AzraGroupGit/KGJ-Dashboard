    ...
    return (
      <div className="flex items-center justify-center h-screen bg-[#26211c]">
        <div className="text-center max-w-md p-8">
          ...
          <h2 className="text-xl font-bold text-cream mb-2">Akses Terbatas</h2>
          <p className="text-white/50">Halaman ini hanya untuk Operational Supervisor.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#26211c]">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
            ...
            <div
              className={`mb-4 p-3 rounded-lg text-sm ${
                message.type === "success"
                  ? "bg-emerald-500/10 text-emerald-300 border border-green-200"
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-xl font-bold text-cream">Slot Management</h1>
              <p className="text-sm text-white/50 mt-1">Kelola kapasitas produksi per kategori</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="border border-gold/25 rounded-lg px-3 py-2 text-sm bg-cocoa"
              />
            </div>
          </div>

          {/* Kategori Table */}
          <div className="bg-cocoa rounded-xl border border-gold/15 overflow-hidden mb-6">
            <div className="px-4 py-3 border-b border-gold/10 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-cream">Daftar Kategori</h2>
              <button
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gold/10 bg-[#26211c]">
                    <th className="text-left px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Kategori</th>
                    <th className="text-left px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Lead Time (hari)</th>
                    <th className="text-center px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Max Slot</th>
                    <th className="text-center px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Terpakai</th>
                    <th className="text-center px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Tersedia</th>
                    <th className="text-center px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Usage</th>
                    <th className="text-right px-4 py-3 font-medium text-white/50 text-xs uppercase tracking-wider">Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  {categories.map((cat) => {
                    ...
                    const barColor =
                      usagePct >= 100 ? "bg-rose-500/100" : usagePct >= 80 ? "bg-amber-500/100" : "bg-emerald-500/100";
                    return (
                      <tr key={cat.id} className="border-b border-gray-50 hover:bg-[#26211c]/50 transition-colors">
                        <td className="px-4 py-3">
                          ...
                          <span className="ml-2 text-[11px] text-white/40 uppercase">{cat.key}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-center text-cream font-medium">{cat.max_slots}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`font-medium ${cat.used >= cat.total_slots ? "text-rose-300" : "text-cream"}`}>
                            {cat.used}
                          </span>
                          {cat.overrides > 0 && (
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`font-medium ${cat.available <= 0 ? "text-rose-300" : cat.available <= 2 ? "text-amber-300" : "text-emerald-300"}`}>
                            {cat.available}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] text-white/50 w-8 text-right">{usagePct}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              ...
                              className="p-1.5 rounded-lg hover:bg-indigo-50 text-white/40 hover:text-indigo-600 transition-colors"
                              title="Edit kategori"
                            >
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg hover:bg-amber-500/100/10 text-white/40 hover:text-amber-300 transition-colors"
          ...
          {showOverrides && overrides.length > 0 && (
            <div className="bg-cocoa rounded-xl border border-gold/15 overflow-hidden mb-6">
              <div className="px-4 py-3 border-b border-gold/10">
                <h2 className="text-sm font-semibold text-cream">Daftar Override</h2>
              </div>
                ...
                {overrides.map((ov) => (
                  <div key={ov.id} className="px-4 py-2.5 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-rose-500/100/10 text-white/40 hover:text-rose-300 transition-colors"
                    >
          ...
          {/* Keterangan */}
          <div className="bg-amber-500/10 border border-amber-400/20 rounded-xl p-4 text-sm text-amber-800">
      ...
      {editModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => !saving && setEditModal(null)}>
          <div className="bg-cocoa rounded-xl shadow-xl border border-gold/15 w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-cream mb-1">Edit Kategori</h3>
            <p className="text-sm text-white/50 mb-4">{editModal.label}</p>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-white/50 mb-1">Lead Time Min (hari kerja)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-white/50 mb-1">Lead Time Max (hari kerja)</label>
                <input type="number" min={1} value={editLeadMax} onChange={(e) => setEditLeadMax(Number(e.target.value))}
                  className="w-full border border-gold/25 rounded-lg px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-white/50 mb-1">Max Slot per Hari</label>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setEditModal(null)} disabled={saving}
                className="flex-1 px-4 py-2 text-sm border border-gold/25 rounded-lg hover:bg-[#26211c] transition-colors disabled:opacity-50">
                Batal
              </button>
      ...
      {forceModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => !saving && setForceModal(null)}>
          <div className="bg-cocoa rounded-xl shadow-xl border border-gold/15 w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-cream mb-1">Tambah Slot (Override)</h3>
            <p className="text-sm text-white/50 mb-4">
              {forceModal.category.label} — {new Date(selectedDate).toLocaleDateString("id-ID")}
            </p>
            <div>
              <label className="block text-xs font-medium text-white/50 mb-1">Catatan (opsional)</label>
              <textarea value={forceNote} onChange={(e) => setForceNote(e.target.value)}
                className="w-full border border-gold/25 rounded-lg px-3 py-2 text-sm resize-none h-20"
